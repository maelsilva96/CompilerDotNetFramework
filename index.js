...
async function SetVariablePath () {
...
try {
    ...
    arrayItems.push(async () => await exec.exec('./VsWhere.exe -latest -requires Microsoft.Component.MSBuild -find MSBuild\\**\\Bin\\MSBuild.exe'));
    arrayItems.push(async () => await SetVariablePath);
    arrayItems.push(async () => await exec.exec('mkdir C:\\_build'));
    arrayItems.push(async () => await exec.exec('mkdir C:\\App'));
    arrayItems.push(async () => await exec.exec(`|\nXcopy ${pathFolderMainProject} C:\\App /E /H /C /I`));
    arrayItems.push(async () => await exec.exec(`nuget restore C:\\App\\${projectFilePathAndName}`));
    ...
    if (compactFile == true || compactFile == 'true') {
        arrayItems.push(async () => await exec.exec(`powershell Compress-Archive C:\\_build\* .\\${nameFileCompact}`));
    } else {
        arrayItems.push(async () => await exec.exec(`mkdir .\\_build`));
        arrayItems.push(async () => await exec.exec(`|\nXcopy C:\\_build .\\_build /E /H /C /I`));
    }
    arrayItems.push(async () => await exec.exec('rmdir /Q /S C:\\_build'));
    arrayItems.push(async () => await exec.exec('rmdir /Q /S C:\\App'));

    Promise.all(arrayItems, (values) => {
        console.log(values);
    });
